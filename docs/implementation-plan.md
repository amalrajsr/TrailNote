...
| `profiles` | `userId` PK/FK user; editable `displayName`; unique lowercase `username`; `role` traveler/moderator default traveler; `status` active/suspended; createdAt/updatedAt. Role/status never client writable |
| `username_claims` | case-insensitive unique username; nullable userId FK for current profile ownership or intentional reservations; createdAt. User-owned claims are released on rename and account deletion |
...
`ContributionCardDTO`: id, destination slug/name, category, derived title, body excerpt, price `{paise,currency,unit,unitLabel}` or null, visitedMonth, lastConfirmedMonth, freshness state, distinct confirmation/helpful counts, changeReported, author `{id,displayName,username,initial,avatar}`, optional first photo `{path,width,height,alt}`, revision, createdAt. Only explicit fields are serialized. Use a verified custom avatar when present and initials otherwise; never expose the Google avatar URL.
