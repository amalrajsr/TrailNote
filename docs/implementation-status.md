...
| W08 — own content, reports, contacts and moderation  | In progress | `/me`, editable unique public identity and avatar, stable public profiles, author edit/delete, contact reveal/removal, reports, moderation views and history, hide/suspend plus restore controls, support scripts and policy pages         | Account-erasure's real ImageKit delete/purge requires provider credentials and was not invoked; profile screenshots remain to be captured                      |
| W09 — resilient behavior and operational integration | In progress | Atomic persistent rate limits, Vercel daily cleanup configuration, authenticated retrying cleanup, redacted logs, report-only CSP, canonical metadata, dynamic sitemap and robots; 4 focused integration tests                             | Vercel staging deployment/manual cron invocation and CSP review remain unrun; monitoring, backup/restore/rollback and incident runbook are deferred by request |
...
- Report queues contain report context and targets but no reporter email or contact
  number. The UI separates open and resolved work, displays action history, confirms
  destructive hide/suspend actions, and exposes tip/account restore controls.
- Resolution and status mutations include the status the moderator saw. Conditional
  updates reject stale repeat actions with `CONFLICT` and refresh the queue; focused
  integration coverage proves resolved records remain visible with their resolution
  notes and that suspend/restore changes public visibility immediately.
...
- `corepack pnpm format:check`, `corepack pnpm lint`, `corepack pnpm typecheck`,
  ...
  locally.
- The focused moderation service file passes 11 integration tests. W08 screenshots
  were intentionally omitted by request; the external ImageKit erasure check remains
  blocked on provider credentials, so W08 remains in progress.

## W09 current verification

- Persistent allowance decisions now use a conditional SQLite conflict update, so a
  rejected request does not increment the counter. Contribution create/edit,
  confirmations/Helpful, reports, uploads, contact reveal/removal, and destination
  suggestions use the documented server-side limits; contribution idempotency is
  checked before charging a replay.
- Cleanup remains lease-based and retryable, adds expiry cleanup for mutation
  receipts/rate-limit buckets, retains deleted contribution families for 30 days,
  and is configured for daily Vercel Cron invocation at `03:00 UTC` through an
  authenticated GET route.
- Public metadata now has canonical URLs, a dynamic sitemap restricted to enabled
  destinations and visible root tips from active travelers, and robots exclusions
  for APIs and private routes. A report-only CSP is configured; no monitoring SDK
  was added.
- `corepack pnpm lint`, `corepack pnpm typecheck`, unit tests (12), integration
  tests (29), local Chromium E2E tests (26), and the webpack production build pass.
  `format:check` remains blocked by pre-existing formatting differences in unrelated
  files, including the existing icon, moderation pages/components, and Drizzle
  snapshots.
- The Vercel project has not yet deployed this configuration, so cron registration,
  an authenticated staging invocation, and CSP browser review are still required.
  Monitoring, backup/restore/rollback, and incident documentation remain deferred by
  the current requested scope.
